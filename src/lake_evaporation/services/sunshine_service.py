...
class SunshineService:
    ...
    def calculate_sunshine_hours(
        ...
        2. Calculate from global radiation (Ångström-Prescott)
        3. Calculate from cloud layers (NWP method)
        ...
        # OPTION 2: Global radiation (Ångström-Prescott)
        if "global_radiation" in data and data["global_radiation"]:
            return self._calculate_from_radiation(data["global_radiation"], latitude, day_number)

        # OPTION 3: Cloud layers (NWP)
        if self._has_cloud_layer_data(aggregates):
            return self._calculate_from_cloud_layers(aggregates, latitude, day_number)

        # FALLBACK: Temperature range (Hargreaves)
        # if self._has_temperature_data(aggregates):
        #    return self._calculate_from_temperature(aggregates, latitude, day_number, location_info)

        # NO DATA AVAILABLE
        self.logger.error("Cannot calculate sunshine hours - insufficient data")
        return 0.0

    def _calculate_from_radiation(
        self,
        radiation_data: List[List[Any]],
        latitude: float,
        day_number: int
    ) -> float:
        """Calculate from global radiation sensor."""
    ...
    def _calculate_from_cloud_layers(
        self,
        aggregates: Dict[str, float],
        latitude: float,
        day_number: int
    ...
    def _calculate_from_temperature(
        self,
        aggregates: Dict[str, float],
        latitude: float,
        day_number: int,
        ...
        """Calculate from temperature range (fallback)."""
        self.logger.warning(
